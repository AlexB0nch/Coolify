...
3. В панели, вкладка Advanced у приложения: включить "Stop before deploy".
   Это единственная настройка из чек-листа, которой нет в API, а без неё во
   время деплоя полминуты живут два воркера и разбирают одни и те же файлы.
